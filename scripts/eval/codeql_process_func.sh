...
find "$ROOT_DIR" -type f -path "*_fixed*" | while read -r code_path; do
    ...
    if [[ "$code_path" == *"out_v2/code/"* ]]; then
        ...
        OUTPUT_PATH="$CVE_DIR/out_v2/code/${base_name}_all_functions.csv"
        
    else
        CVE_DIR=$(dirname "$code_path")
        OUTPUT_PATH="$CVE_DIR/out_v2/${base_name}_all_functions.csv"
    fi
    ...
    temp_csv_file=tmp_result_func_"$base_name"_"$function_name".csv

    # Process function list
    echo "[INFO] Processing function list: $CVE_NAME => $OUTPUT_PATH"
    original_query_file="${QUERY_DIR}/function_extraction.ql"
    temp_query_file="${QUERY_DIR}/function_extraction_${base_name}_${function_name}.ql"
