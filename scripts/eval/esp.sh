...
for cve_dir in "$cve_root"*/; do
    ...
    if [ -d "$cve_dir" ] && [[ "$cve_dir" != *"EMPTY_CVE"* ]] && [[ "$cve_dir" != *"BACKUP"* ]]; then
        ...
        cwe_type=$(jq -er '.cwe_id' "$info_file")
        cwe_file="CWE-list/$cwe_type.json"
        ...
        for code_file in "$cve_dir"out_v2/code/*.json; do
            ...
            output_dir="$cve_dir/out_v2/eval_simple_cwe_patch"

            code_filename=$(basename "$code_file")
            code_filename="${code_filename%.*}"

            verify_file="$cve_dir""out_v2/eval_simple_cwe_verification/verify_$model($code_filename).json"

            inputs="$code_file,$cwe_file,$verify_file"
            ...
            if jq -e '.result == true' $verify_file > /dev/null; then
                ...
                if $use_parallel; then
                    wait_for_jobs
                    python src/main.py -esp "$inputs" -o "$output_dir" -m "$model" &
                else
                    python src/main.py -esp "$inputs" -o "$output_dir" -m "$model"
                    if [ $? -eq 255 ]; then
