    ...
    if [ -d "$cve_dir" ] && [[ "$cve_dir" != *"EMPTY_CVE"* ]] && [[ "$cve_dir" != *"BACKUP"* ]]; then
        ...
        for code_file in "$cve_dir"out_v2/code/*.json; do
            ...
            if jq -e 'has("is_vulnerable")' "$code_file" >/dev/null; then
                ...
                if [ "$value" == "N/A" ] || [ "$value" == "false" ]; then
                    echo "Skipping because $code_file has is_vulnerable set to N/A"
                    continue
                fi
            fi

            output_dir="$cve_dir/out_v2/eval_patch"

            code_filename=$(basename "$code_file")
            code_filename="${code_filename%.*}"

            db_search_file="$cve_dir""out_v2/eval_verification/db_search_$model($code_filename).json"
            verify_file="$cve_dir""out_v2/eval_verification/verify_$model($code_filename).json"

            inputs="$code_file,$db_search_file,$verify_file"
            ...
            if jq -e '.result == true' $verify_file > /dev/null; then
                ...
                if $use_parallel; then
                    wait_for_jobs
                    python src/main.py -ep-v2 "$inputs" -o "$output_dir" -m "$model" &
                else
                    python src/main.py -ep-v2 "$inputs" -o "$output_dir" -m "$model"
                    if [ $? -eq 255 ]; then
