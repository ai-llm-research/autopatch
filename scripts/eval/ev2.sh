...
for cve_dir in "$cve_root"*/; do
    ...
    if [ -d "$cve_dir" ] && [[ "$cve_dir" != *"EMPTY_CVE"* ]] && [[ "$cve_dir" != *"BACKUP"* ]]; then
        ...
        for code_file in "$cve_dir"out_v2/code/*.json; do
            ...
            data_flow_file="$cve_dir/out_v2/code/${code_filename}_data_flow.csv"
            inputs="$code_file,$data_flow_file"
            # Check if the output directory already exists
            if [ -f "$output_dir/taint_variable_$model($code_filename).json" ]; then
                echo "Skipping $cve_dir because taint_variable_$model($code_filename).json exists."
            ...
            if $use_parallel; then
                wait_for_jobs
                python src/main.py -ev2 "$inputs" -o "$output_dir" -m "$model" &
            else
                python src/main.py -ev2 "$inputs" -o "$output_dir" -m "$model"
                if [ $? -eq 255 ]; then
