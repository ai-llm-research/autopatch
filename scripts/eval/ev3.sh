...
for cve_dir in "$cve_root"*/; do
    ...
    if [ -d "$cve_dir" ] && [[ "$cve_dir" != *"EMPTY_CVE"* ]] && [[ "$cve_dir" != *"BACKUP"* ]]; then
        ...
        for code_file in "$cve_dir"out_v2/code/*.json; do
            ...
            if [ -f "$output_dir/taint_function_$model($code_filename).json" ]; then
                echo "Skipping $cve_dir because taint_function_$model($code_filename).json exists."
            ...
            if $use_parallel; then
                wait_for_jobs
                python src/main.py -ev3 "$inputs" -o "$output_dir" -m "$model" &
            else
                python src/main.py -ev3 "$inputs" -o "$output_dir" -m "$model"
                if [ $? -eq 255 ]; then
